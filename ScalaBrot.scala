// Persistent Mandelbrot worker: reads "width height maxIter\n" on stdin,
// writes width*height uint16 little-endian counts (row-major) on stdout.
// Count = iterations survived; in-set pixels = maxIter.
// SIMD via jdk.incubator.vector, rows parallelized across all cores.

import jdk.incubator.vector.{FloatVector, IntVector, VectorMask, VectorOperators, VectorSpecies}
import java.io.{BufferedOutputStream, BufferedReader, FileDescriptor, FileOutputStream, InputStreamReader}
import java.util.stream.IntStream

object ScalaBrot:
  private val FS: VectorSpecies[java.lang.Float] = FloatVector.SPECIES_128
  private val IS: VectorSpecies[java.lang.Integer] = IntVector.SPECIES_128

  def main(args: Array[String]): Unit =
    val reader = new BufferedReader(new InputStreamReader(System.in))
    // Raw fd 1 for binary output; never touch System.out.
    val out = new BufferedOutputStream(new FileOutputStream(FileDescriptor.out), 1 << 20)
    var line = reader.readLine()
    while line != null do
      val t = line.trim
      if t.nonEmpty then
        val p = t.split("\\s+")
        out.write(compute(p(0).toInt, p(1).toInt, p(2).toInt))
        out.flush()
      line = reader.readLine()

  def compute(width: Int, height: Int, maxIter: Int): Array[Byte] =
    val frame = new Array[Byte](width * height * 2)
    val dx = if width > 1 then 3.5 / (width - 1) else 0.0
    val dy = if height > 1 then 2.0 / (height - 1) else 0.0
    val crArr = new Array[Float](width)
    var i = 0
    while i < width do
      crArr(i) = (-2.5 + i * dx).toFloat
      i += 1

    val half = (height + 1) / 2 // y-symmetry: bottom half mirrors top
    IntStream.range(0, half).parallel().forEach { row =>
      val ci = (-1.0 + row * dy).toFloat
      val counts = new Array[Int](width)
      computeRow(crArr, ci, maxIter, counts)
      val off = row * width * 2
      var x = 0
      while x < width do
        val v = counts(x)
        frame(off + 2 * x) = v.toByte
        frame(off + 2 * x + 1) = (v >>> 8).toByte
        x += 1
      val mrow = height - 1 - row
      if mrow != row then
        System.arraycopy(frame, off, frame, mrow * width * 2, width * 2)
    }
    frame

  private def computeRow(crArr: Array[Float], ci: Float, maxIter: Int, counts: Array[Int]): Unit =
    val width = crArr.length
    val civ = FloatVector.broadcast(FS, ci)
    val ci2v = civ.mul(civ)
    val lanes = FS.length()
    val upper = width - width % lanes
    var x = 0
    while x < upper do
      val cr = FloatVector.fromArray(FS, crArr, x)

      // cardioid + period-2 bulb early-out
      val crm = cr.sub(0.25f)
      val q = crm.mul(crm).add(ci2v)
      var inSet: VectorMask[java.lang.Float] =
        q.mul(q.add(crm)).compare(VectorOperators.LE, ci2v.mul(0.25f))
      val crp = cr.add(1.0f)
      inSet = inSet.or(crp.mul(crp).add(ci2v).compare(VectorOperators.LE, 0.0625f))

      var count = IntVector.zero(IS).blend(maxIter.toLong, inSet.cast(IS))
      var active = inSet.not()
      if active.anyTrue() then
        var zr = FloatVector.zero(FS)
        var zi = FloatVector.zero(FS)
        var it = 0
        var go = true
        while go && it < maxIter do
          val zr2 = zr.mul(zr)
          val zi2 = zi.mul(zi)
          active = active.andNot(zr2.add(zi2).compare(VectorOperators.GT, 4.0f))
          if !active.anyTrue() then go = false
          else
            val nzr = zr2.sub(zi2).add(cr)
            zi = zr.add(zr).fma(zi, civ) // 2*zr*zi + ci
            zr = nzr
            count = count.sub(-1, active.cast(IS)) // +1 on active lanes
            it += 1
      count.intoArray(counts, x)
      x += lanes
    while x < width do
      counts(x) = scalarPixel(crArr(x), ci, maxIter)
      x += 1

  private def scalarPixel(cr: Float, ci: Float, maxIter: Int): Int =
    val crm = cr - 0.25f
    val ci2 = ci * ci
    val q = crm * crm + ci2
    if q * (q + crm) <= 0.25f * ci2 then return maxIter
    val crp = cr + 1.0f
    if crp * crp + ci2 <= 0.0625f then return maxIter
    var zr = 0.0f
    var zi = 0.0f
    var it = 0
    var go = true
    while go && it < maxIter do
      val zr2 = zr * zr
      val zi2 = zi * zi
      if zr2 + zi2 > 4.0f then go = false
      else
        val nzr = zr2 - zi2 + cr
        zi = 2.0f * zr * zi + ci
        zr = nzr
        it += 1
    it
