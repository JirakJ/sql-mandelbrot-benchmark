"""
CSharpBrot - C# .NET ARM NEON (AdvSimd) Mandelbrot via a persistent worker process.

dotnet publish and JIT warm-up happen at import time (untimed).
run_csharpbrot only writes a request line and reads raw uint16 pixels back.
...
_PROJ = os.path.join(_DIR, "csharpbrot")
_SRCS = [os.path.join(_PROJ, "Program.cs"), os.path.join(_PROJ, "csharpbrot.csproj")]
_PUB = os.path.join(_PROJ, "bin", "publish")
_DLL = os.path.join(_PUB, "csharpbrot.dll")
...
def _build():
    ...
    if res.returncode != 0:
        raise RuntimeError(
            "csharpbrot: dotnet publish failed:\n"
...
def run_csharpbrot(width, height, max_iterations):
    """Compute the Mandelbrot set in the C# worker. Returns (h, w) uint16."""
    ...
    while got < total:
        ...
        if not n:
            raise RuntimeError("csharpbrot: worker died mid-frame")
        got += n
    return out
...
run_csharpbrot(16, 16, 8)
run_csharpbrot(1400, 800, 256)
run_csharpbrot(1400, 800, 256)
...
if __name__ == "__main__":
    ...
    result = run_csharpbrot(WIDTH, HEIGHT, MAX_ITERATIONS)
    save_mandelbrot_image(result, MAX_ITERATIONS, "csharpbrot.png")
